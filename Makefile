...
$(LIBDIR)/$(ARCHIVE): $(OBJS)
	...
	ar -cvq $@ $(OBJS)
.SECONDEXPANSION:
$(OBJDIR)/%.o: %.dats $$(wildcard src/SATS/$$*.sats) deps/ats-result
	$(dir_guard)
...
$(EXEDIR)/%: $(TESTDIR)/%.dats $(LIBDIR)/$(ARCHIVE)
	$(dir_guard)
	PATSRELOCROOT=$(PWD) $(ATSCC) $(ATSCCFLAGS) -o $@ $< -cleanaft $(LIBDIR)/$(ARCHIVE) ./deps/ats-result/lib/libatsresult.a
	$(EXEDIR)/$(APP)
installdeps: deps
deps: deps/ats-result
	$(dir_guard)
deps/ats-result:
	git clone http://ubuntu-netfu:3000/randy.valis/ats-result $@
	make -C $@
.SILENT: run 
